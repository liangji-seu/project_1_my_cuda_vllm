...
from model_qwen3 import ModelArgs, Transformer
...
def legacy_export(model, filepath):
    ...
    head_dim = p.head_dim if p.head_dim > 0 else p.dim // p.n_heads
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-seq-len", default=None, type=int, help="override max sequence length")
    ...
    if args.max_seq_len is not None:
        model.params.max_seq_len = args.max_seq_len
        model.freqs_cos = model.freqs_cos[:args.max_seq_len]
        model.freqs_sin = model.freqs_sin[:args.max_seq_len]
        print(f"Overriding max_seq_len to {args.max_seq_len}")
